...
const Home = () => {
    const {data:journals, isPending, error} = useFetch('http://localhost:8000/journals');
    ...
    return  (
        <div className="px-6 pb-10 overflow-y-auto overflow-x-hidden bg-slate-800 w-100">
            ...
            {error && <div className="font-poppins"> {error} </div>}
                {!isPending && journals.length === 0 && <div className="font-poppins text-sm text-slate-500/50"> No journals </div>}
